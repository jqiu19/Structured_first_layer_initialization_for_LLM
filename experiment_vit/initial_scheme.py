...
def init_qkv_with_custom_gaussian_and_bias(attn_module, model, args):
    return init_qkv_with_custom_gaussian_and_bias_qkv(attn_module, model, args)


def add_qkv_activation(attn_module, args):
    return add_qkv_activation_qkv(attn_module, args)


def _is_llama_style_model(model):
    return hasattr(model, "model") and hasattr(model.model, "layers") and hasattr(model.model, "embed_tokens")


def _is_vit_style_model(model):
...
def _get_vit_patch_embed_module(model):
    patch_embed = getattr(model, "patch_embed", None)
    if patch_embed is None:
        return None
    proj = getattr(patch_embed, "proj", None)
    if isinstance(proj, (nn.Conv2d, nn.Linear)):
        return proj
    return None


def _replace_module_with_bias(parent, name):
    module = getattr(parent, name)
    if isinstance(module, nn.Linear) and module.bias is None:
        module = _replace_linear_with_bias(module)
        setattr(parent, name, module)
    return module


def _init_weight_module(module, init_fn):
    with torch.no_grad():
...
def _apply_constructed_weight(module, new_weight):
    with torch.no_grad():
        if module.weight.ndim == 2:
            module.weight.copy_(new_weight.to(dtype=module.weight.dtype, device=module.weight.device))
            return

        flat_shape = (module.weight.shape[0], module.weight[0].numel())
        reshaped = new_weight.reshape(flat_shape).reshape_as(module.weight)
        module.weight.copy_(reshaped.to(dtype=module.weight.dtype, device=module.weight.device))


def _structured_gaussian_with_bias_vit(module, args, prefix="sfli"):
    if module.bias is None:
        raise RuntimeError("structured ViT initialization expects a linear layer with bias")

    std = float(
        getattr(
            args,
            f"{prefix}_std",
            getattr(args, "ffn_base_std", getattr(args, "v_qkv_std", 0.02)),
        )
    )
    scale = float(
        getattr(
            args,
            f"{prefix}_hd_coeffs",
            getattr(args, "ffn_hd_coeffs", getattr(args, "v_hd_coeffs", 1.0)),
        )
    )
    bias_start = float(getattr(args, "ffn_bias_start", getattr(args, "v_bias_start", 0.0)))
    bias_step = float(
        getattr(
            args,
            f"{prefix}_bias_step",
            getattr(args, "ffn_bias_step", getattr(args, "v_bias_step", 0.0)),
        )
    )
    bias_noise_std = float(
        getattr(
            args,
            f"{prefix}_bias_noise_std",
            getattr(args, "ffn_bias_noise_std", getattr(args, "v_bias_noise_std", 0.0)),
        )
    )
    ...
    with torch.no_grad():
        ...
        arith = bias_start + bias_step * torch.arange(
            out_dim, device=module.weight.device, dtype=module.weight.dtype
        )
        noise = torch.randn(out_dim, device=module.weight.device, dtype=module.weight.dtype) * bias_noise_std
        module.bias.copy_(arith + noise)


def _add_activation_to_vit_qkv(attn_module, args):
    act_name = getattr(args, "qkv_act", "none")
    if act_name == "none":
        return

    qkv = getattr(attn_module, "qkv", None)
    if isinstance(qkv, nn.Sequential) or not isinstance(qkv, nn.Linear):
        return

    setattr(attn_module, "qkv", nn.Sequential(qkv, _make_act(act_name)))


def _vit_apply_patch_embed_init(model, init_fn):
    patch_proj = _get_vit_patch_embed_module(model)
    if patch_proj is not None:
        _init_weight_module(patch_proj, init_fn)


def _vit_apply_patch_embed_constructed(model, fn, args):
    patch_proj = _get_vit_patch_embed_module(model)
    if patch_proj is None:
        return

    flat_weight = patch_proj.weight.detach().reshape(patch_proj.weight.shape[0], -1)
    new_weight = fn(args, flat_weight)
    _apply_constructed_weight(patch_proj, new_weight)


def _vit_init_mlp(module, args):
    _structured_gaussian_with_bias_vit(module, args, prefix="sfli")


def _vit_apply_ffn_structured(model, args, which="both"):
    for block in _iter_vit_blocks(model):
        mlp = getattr(block, "mlp", None)
        if mlp is None:
            continue

        if which in ("fc1", "both"):
            fc1 = _replace_module_with_bias(mlp, "fc1")
            if isinstance(fc1, nn.Linear):
                _vit_init_mlp(fc1, args)

        if which in ("fc2", "both"):
            fc2 = _replace_module_with_bias(mlp, "fc2")
            if isinstance(fc2, nn.Linear):
                _vit_init_mlp(fc2, args)


def _vit_apply_qkv_structured(model, args, add_activation=False):
    for block in _iter_vit_blocks(model):
        attn = getattr(block, "attn", None)
        if attn is None:
            continue

        qkv = _replace_module_with_bias(attn, "qkv")
        if isinstance(qkv, nn.Linear):
            _structured_gaussian_with_bias_vit(qkv, args, prefix="sfli")
            if add_activation:
                _add_activation_to_vit_qkv(attn, args)


def _vit_apply_xavier_all_ffn(model):
    for block in _iter_vit_blocks(model):
        mlp = getattr(block, "mlp", None)
        if mlp is None:
            continue
        for name in ("fc1", "fc2"):
            ...
            if isinstance(layer, nn.Linear):
                _init_weight_module(layer, nn.init.xavier_uniform_)


def _vit_apply_kaiming_all_ffn(model):
    for block in _iter_vit_blocks(model):
        mlp = getattr(block, "mlp", None)
        if mlp is None:
            continue
        for name in ("fc1", "fc2"):
            layer = getattr(mlp, name, None)
            if isinstance(layer, nn.Linear):
                _init_weight_module(layer, nn.init.kaiming_uniform_)


def _apply_vit_initialization(model, args):
    method = getattr(args, "initialization", None)

    if method == "Xavier":
        _vit_apply_patch_embed_init(model, nn.init.xavier_uniform_)
        _vit_apply_xavier_all_ffn(model)
        return model

    if method == "Kaiming":
        _vit_apply_patch_embed_init(model, nn.init.kaiming_uniform_)
        _vit_apply_kaiming_all_ffn(model)
        return model

    if method == "erank_kernel":
        _vit_apply_patch_embed_constructed(model, kernel_fun, args)
        return model

    if method == "erank_basis":
        _vit_apply_patch_embed_constructed(model, basis_fun, args)
        return model

    if method == "erank_implicit":
        _vit_apply_patch_embed_constructed(model, implicit_fun, args)
        _vit_apply_qkv_structured(model, args, add_activation=True)
        return model

    if method == "Xavier_plus_SFLI":
        _vit_apply_patch_embed_init(model, nn.init.xavier_uniform_)
        _vit_apply_qkv_structured(model, args, add_activation=True)
        return model

    if method == "Kaiming_plus_SFLI":
        _vit_apply_patch_embed_init(model, nn.init.kaiming_uniform_)
        _vit_apply_qkv_structured(model, args, add_activation=True)
        return model

    if method == "Xavier_FFN_SFLI":
        _vit_apply_patch_embed_init(model, nn.init.xavier_uniform_)
        _vit_apply_ffn_structured(model, args, which="both")
        return model

    if method == "Kaiming_FFN_SFLI":
        _vit_apply_patch_embed_init(model, nn.init.kaiming_uniform_)
        _vit_apply_ffn_structured(model, args, which="both")
        return model

    if method == "Gaussian_default":
        return model

    if method == "Gaussian_plus_SFLI":
        _vit_apply_qkv_structured(model, args, add_activation=True)
        return model

    if method in ("gaussian_FFN_SFLI", "gaussian_FFN_SFLI_ugd", "gaussian_FFN_SFLI_ud", "gaussian_FFN_SFLI_gd"):
        _vit_apply_ffn_structured(model, args, which="both")
        return model

    if method in ("gaussian_FFN_SFLI_g", "gaussian_FFN_SFLI_u"):
        _vit_apply_ffn_structured(model, args, which="fc1")
        return model

    if method == "gaussian_FFN_SFLI_d":
        _vit_apply_ffn_structured(model, args, which="fc2")
        return model

    if method == "gaussian_best_combination":
        _vit_apply_qkv_structured(model, args, add_activation=True)
        _vit_apply_ffn_structured(model, args, which="fc1")
        return model

    raise ValueError(f"Unknown ViT initialization method: {method}")


def apply_initialization(model, args):
    method = getattr(args, "initialization", None)
    if method in (None, "default", "timm_default"):
        return model

    if _is_llama_style_model(model):
        return initial_embedding(model, args)

    if _is_vit_style_model(model):
        return _apply_vit_initialization(model, args)

    raise ValueError(f"Unsupported model structure for initialization method {method}")
#

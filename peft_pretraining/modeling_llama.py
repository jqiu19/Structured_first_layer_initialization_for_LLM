...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_index: int):
        ...
        self.x_std_ema = None
        self.lns_base_coeff = float(os.getenv("LNS_BASE_COEFF", "1.0"))
        self.lns_control_period = max(1, int(os.getenv("LNS_CONTROL_PERIOD", "20")))
        self.lns_ref_var_min = float(os.getenv("LNS_REF_VAR_MIN", str(0.65 ** 2)))
        self.lns_ref_var_max = float(os.getenv("LNS_REF_VAR_MAX", str(1.75 ** 2)))
        self.attn_adaptive_scale_start = 1.0
        self.attn_adaptive_scale_target = 1.0
        self.mlp_adaptive_scale_start = 1.0
        self.mlp_adaptive_scale_target = 1.0
        self.attn_adaptive_scale = 1.0
        self.mlp_adaptive_scale = 1.0
        ...
        if norm_type == 'pre' or norm_type == 'scale_pre' or norm_type == 'LNS' or norm_type == 'lns' or norm_type == 'outer_lns' or norm_type == 'outer_lns1' or norm_type == 'outer_LNS' or norm_type == 'outer_LNS1':
    ...
    def forward(
        ...
        if norm_type == 'pre' or norm_type == 'scale_pre':
            ...
            scale_factor = 1 / ((self.layer_index + 1) ** 0.5)
        ...
        elif norm_type == 'outer_lns':
            period = max(getattr(self, "lns_control_period", 20), 1)
            layer_frac = self.layer_index / max(self.layer_nums - 1, 1)
            target_var_min = float(getattr(self, "lns_ref_var_min", 0.65 ** 2))
            target_var_max = float(getattr(self, "lns_ref_var_max", 1.75 ** 2))
            target_var = target_var_min + (target_var_max - target_var_min) * layer_frac
            target_std = math.sqrt(max(target_var, 1e-12))

            if (
                self.training
                and getattr(self, "x_std_ema", None) is not None
                and self.current_step > 0
                and self.current_step % period == 0
            ):
                eta = 0.05
                error = math.log(max(float(self.x_std_ema), 1e-6) / target_std)

                self.attn_adaptive_scale *= math.exp(-eta * error)
                self.mlp_adaptive_scale *= math.exp(-(eta * 1.15) * error)

                self.attn_adaptive_scale = float(min(max(self.attn_adaptive_scale, 0.1), 1.5))
                self.mlp_adaptive_scale = float(min(max(self.mlp_adaptive_scale, 0.1), 1.5))

            base_scale = float(getattr(self, "lns_base_coeff", 1.0)) / math.sqrt(self.layer_index + 1)
            attn_scale_factor = base_scale * self.attn_adaptive_scale
            mlp_scale_factor = base_scale * self.mlp_adaptive_scale
            ...
            hidden_states = attn_scale_factor * hidden_states
            hidden_states = residual + hidden_states
            ...
            hidden_states = self.post_attention_layernorm(hidden_states)
            hidden_states = self.mlp(hidden_states)
            hidden_states = mlp_scale_factor * hidden_states
            hidden_states = residual + hidden_states

        
        elif norm_type == 'outer_lns1' or norm_type == 'outer_LNS1':
            step_ratio = self.current_step / max(self.total_steps, 1)

            # 温和的动态基线：前期别压太狠，后期逐渐增强
            d_base = 0.5 + 0.10 * (1.0 - math.exp(-5.0 * step_ratio))
            time_scale = 1.0 + 1.8 * (1.0 - math.exp(-4.0 * step_ratio))

            # 放宽目标曲线：bottom≈0.65, top≈1.75
            layer_frac = self.layer_index / max(self.layer_nums - 1, 1)
            target_std = 0.65 + 1.10 * layer_frac

            # 每 20 step 用上一轮 EMA std 小步更新一次
            if (
                self.training
                and self.x_std_ema is not None
                and self.current_step > 0
                and self.current_step % 20 == 0
                and self.current_step != self.last_adapt_step
            ):
                eta = 0.05
                error = math.log(max(self.x_std_ema, 1e-6) / target_std)

                # attention 和 mlp 分开调，mlp 稍微更积极一点
                self.attn_adaptive_scale *= math.exp(-eta * error)
                self.mlp_adaptive_scale *= math.exp(-(eta * 1.15) * error)
                ...
                self.last_adapt_step = self.current_step

            base_scale = 1.0 / (((self.layer_index + 1) ** d_base) * time_scale)
            attn_scale_factor = base_scale * self.attn_adaptive_scale
            mlp_scale_factor = base_scale * self.mlp_adaptive_scale
